...
const PLUGIN_COLORS  = { sentinel: C.yellow, tribunal: C.pink, archivist: C.cyan, scribe: C.purple };
const PLUGIN_LABELS  = { sentinel: "SEN", tribunal: "TRB", archivist: "ARC", scribe: "SCR" };
const STATUS_COLORS  = { pass: C.green, warn: C.yellow, fail: C.red, info: C.cyan, block: C.red };
...
function MiniBar({ value, color, width = 60 }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      <div style={{ width, height: 3, borderRadius: 2, background: C.bg3, overflow: "hidden", flexShrink: 0 }}>
        <div style={{
...
function ScoreGauge({ score }) {
  const r = 28, circ = 2 * Math.PI * r, fill = circ * score;
  const color = scoreColor(score);
  return (
    <div style={{ position: "relative", width: 72, height: 72, flexShrink: 0 }}>
      <svg width={72} height={72} style={{ transform: "rotate(-90deg)" }}>
        <circle cx={36} cy={36} r={r} fill="none" stroke={C.bg3} strokeWidth={5} />
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column",
        ...
        <span style={{ fontSize: 15, fontWeight: 700, color, fontFamily: "'JetBrains Mono', monospace", lineHeight: 1 }}>
          {score.toFixed(2)}
        </span>
        <span style={{ fontSize: 8, color: C.textMuted, marginTop: 1 }}>score</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
      <div style={{ padding: "8px 14px 4px", display: "flex", alignItems: "center", gap: 6 }}>
        ...
        <span style={{ fontSize: 10, color: C.textMuted }}>{events.length} events this session</span>
      </div>

      <div ref={ref} style={{ flex: 1, overflowY: "auto", padding: "4px 0" }}>
        {events.length === 0 && (
          <div style={{ padding: "24px 14px", textAlign: "center", color: C.textMuted, fontSize: 11 }}>
        ...
        {events.map((e, i) => (
          <div key={i} style={{
            ...
            padding: "5px 14px", transition: "background 0.1s", cursor: "default",
            animation: i === events.length - 1 ? "fadeSlideIn 0.25s ease" : undefined,
          }}
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                ...
                <span style={{ fontSize: 11, color: C.textSecondary, fontWeight: 500 }}>{e.label}</span>
...
function MetricsTab({ events }) {
  ...
  const sentinelBlocks  = events.filter((e) => e.plugin === "sentinel" && e.status === "block").length;
  const archivistWrites = events.filter((e) => e.plugin === "archivist" && e.label?.includes("write")).length;
  const warnCount       = events.filter((e) => e.status === "warn" || e.status === "fail").length;
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "14px" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 18 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Session · Today</div>
          {[
            ...
            <div key={m.label} style={{ display: "flex", justifyContent: "space-between",
              fontSize: 11, marginBottom: 5, color: C.textSecondary }}>
              <span>{m.label}</span>
              <span style={{ fontFamily: "monospace", color: C.textPrimary }}>{m.value}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        <div style={{ fontSize: 10, letterSpacing: "0.08em", color: C.textMuted,
        ...
        {["tribunal", "archivist", "sentinel", "scribe"].map((plugin) => {
          ...
          const passRate = pluginEvents.length
            ? pluginEvents.filter((e) => e.status === "pass" || e.status === "info").length / pluginEvents.length
            : 0;
          return (
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 3 }}>
                <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <span style={{ width: 6, height: 6, borderRadius: "50%",
                    background: PLUGIN_COLORS[plugin], display: "inline-block" }} />
                  {plugin.charAt(0).toUpperCase() + plugin.slice(1)}
                </span>
                <span style={{ fontSize: 10, color: C.textMuted }}>{pluginEvents.length} events</span>
              </div>
              <MiniBar value={passRate} color={PLUGIN_COLORS[plugin]} width={120} />
            </div>
          );
        })}
      </div>
      ...
      <div style={{ background: C.bg2, borderRadius: 8, padding: 12, border: `1px solid ${C.border}` }}>
        <div style={{ fontSize: 10, letterSpacing: "0.08em", color: C.textMuted,
          textTransform: "uppercase", fontFamily: "monospace", marginBottom: 10 }}>
        ...
        {[
          ...
          <div key={layer.label} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 7 }}>
            <span style={{ width: 102, fontSize: 9.5, color: C.textSecondary, flexShrink: 0 }}>
              {layer.label}
            </span>
            <MiniBar value={layer.val} color={layer.color} width={80} />
          </div>
        ))}
        <div style={{ fontSize: 9, color: C.textMuted, marginTop: 6, fontStyle: "italic", lineHeight: 1.5 }}>
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "12px 14px" }}>
      ...
      {events.slice(-50).reverse().map((e, i) => (
        <div key={i} style={{
          marginBottom: 6, padding: "8px 10px", background: C.bg2,
          borderRadius: 6, border: `1px solid ${C.border}`,
          fontSize: 10, fontFamily: "monospace", color: C.textMuted,
          lineHeight: 1.6,
        }}>
          <div style={{ display: "flex", gap: 6, marginBottom: 2 }}>
            <span style={{ color: STATUS_COLORS[e.status] ?? C.textMuted }}>●</span>
      ...
      <div style={{ marginTop: 10, padding: 10, background: C.bg0, borderRadius: 6,
        fontSize: 10, fontFamily: "monospace", color: C.textMuted, border: `1px solid ${C.border}` }}>
        <div style={{ color: C.cyan, marginBottom: 4 }}>// ~/.claude/hooks.json (active)</div>
...
export default function ObservabilityPane({ events, settings }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "10px 14px 0", borderBottom: `1px solid ${C.border}` }}>
        <div style={{ display: "flex", alignItems: "center", marginBottom: 8 }}>
          <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em",
            color: C.pink, fontFamily: "monospace", textTransform: "uppercase" }}>
            Onlooker
          </span>
        </div>
        <div style={{ display: "flex", gap: 0 }}>
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{
              fontSize: 10, padding: "5px 12px", border: "none", background: "transparent",
      ...
      <div style={{ padding: "7px 14px", borderTop: `1px solid ${C.border}`,
        display: "flex", gap: 8, fontSize: 9, color: C.textMuted,
        fontFamily: "monospace", background: C.bg0, flexWrap: "wrap" }}>
