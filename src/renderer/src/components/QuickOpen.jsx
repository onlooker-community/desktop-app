...
import { STATUS_COLORS, pluginColor, PLUGIN_LABELS } from "../plugins.js";
  ...
  const results = useMemo(() => {
    ...
    for (const s of sessions.slice(0, 100)) {
      if (s.id?.toLowerCase().includes(q)) {
        items.push({
          ...
          icon: "◎",
          category: "Sessions",
          detail: s.start ? new Date(s.start).toLocaleDateString("en-US", {
    ...
    for (const s of sessions.slice(0, 50)) {
      for (const e of (s.events ?? []).slice(0, 200)) {
        ...
        if (e.label?.toLowerCase().includes(q) || e.detail?.toLowerCase().includes(q)) {
          matchedEvents.push({
            type: "event",
            label: e.label,
    ...
    >
      ...
      >
              ...
              return results.map((item, i) => {
                ...
                return (
                  <div key={`${item.type}-${item.id ?? item.label}-${i}`}>
                    {showCategory && (
                      <div style={{
                        padding: "6px 16px 4px", fontSize: 9,
        ...
        <div style={{
          ...
          display: "flex", gap: 12, fontSize: 9, color: C.textMuted,
