...
import {
  useCostData, useDeadEnds, useFileAttention, useInstructionHealth,
} from "../hooks/useOnlooker.js";
import { pluginColor } from "../plugins.js";
    ...
    <div style={{
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {projects.map((p) => {
          ...
          return (
            <div key={p.cwd} onClick={() => onSelect(p.cwd)} style={{
            ...
            >
              <div style={{
                fontSize: 10, color: C.textPrimary, fontFamily: "monospace",
...
export default function ProjectDashboard({ sessions }) {
  ...
  const { files: fileAttention } = useFileAttention();
  ...
  const projects = useMemo(() => {
    ...
    return Object.values(cwdMap).sort((a, b) => b.sessionCount - a.sessionCount);
  }, [sessions, health?.cwd]);
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 24px" }}>
        {!project ? (
          <div style={{
            ...
            color: C.textMuted, fontSize: 13, height: "100%",
          }}>
            No projects found — sessions will appear once agent activity is detected
          </div>
        ) : (
          <>
            {/* Header */}
              ...
              <div style={{
                ...
                <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
                  <span style={{
                    ...
                    color: health.health_score >= 0.85 ? C.green :
                      health.health_score >= 0.7 ? C.yellow : C.red,
                  }}>
                  ...
                  {health.last_audit_at && (
                    <span style={{ fontSize: 9, color: C.textMuted, fontFamily: "monospace" }}>
                      audited {new Date(health.last_audit_at).toLocaleDateString()}
                    </span>
                  )}
                </div>
              </div>
            ...
            <div style={{
              ...
              {projectSessions.slice(0, 10).map((s) => (
                <div key={s.id} style={{
                  ...
                  <span style={{
                    fontSize: 9, color: C.textMuted, fontFamily: "monospace", width: 80,
                  }}>
                  ...
                  <span style={{
                    fontSize: 10, color: scoreColor(s.avgScore),
                  ...
                  {s.friction != null && (
                    <span style={{
                      fontSize: 9, color: frictionColor(s.friction.score),
                      fontFamily: "monospace", width: 40,
                    }}>
            ...
            {projectDeadEnds.length > 0 && (
              <div style={{
                ...
                {projectDeadEnds.slice(0, 8).map((de, i) => (
                  <div key={i} style={{
                    fontSize: 10, color: C.textSecondary, padding: "4px 0",
