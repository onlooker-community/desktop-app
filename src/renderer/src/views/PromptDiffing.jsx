...
import { useInstructionWatcher, useInstructionHealth } from "../hooks/useOnlooker.js";
...
function diffLines(oldText, newText) {
  ...
  return { changes: result, unchanged, added: result.filter((r) => r.type === "add").length,
...
function DiffView({ change }) {
  ...
  const diff = useMemo(() =>
    diffLines(change.oldContent, change.newContent), [change]);
  ...
  return (
    <div style={{
      ...
      >
        ...
        <span style={{
          fontSize: 11, color: C.textPrimary, fontFamily: "monospace", flex: 1,
        }}>
          {change.file}
        </span>
        ...
        <div style={{
          ...
          {diff.changes.length === 0 ? (
            ...
            diff.changes.map((line, i) => (
              <div key={i} style={{
                padding: "1px 14px",
                background: line.type === "add" ? `${C.green}0a` :
                  line.type === "remove" ? `${C.red}0a` : "transparent",
                color: line.type === "add" ? C.green :
                  line.type === "remove" ? C.red : C.textSecondary,
              }}>
                <span style={{ color: C.textMuted, width: 14, display: "inline-block" }}>
...
export default function PromptDiffing() {
  ...
  const healthPct = health?.health_score != null
    ? `${Math.round(health.health_score * 100)}%` : "—";
  const healthColor = health?.health_score != null
    ? (health.health_score >= 0.85 ? C.green : health.health_score >= 0.7 ? C.yellow : C.red)
    : C.textMuted;
    ...
    <div style={{ height: "100%", overflowY: "auto", padding: "20px 24px" }}>
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{
            ...
            border: `1px solid ${C.pink}44`, color: C.pink,
            background: `${C.pink}11`, fontFamily: "'JetBrains Mono', monospace",
          }}>
            LIVE
          </span>
        </div>
        <span style={{
          fontSize: 11, fontWeight: 700, color: healthColor, fontFamily: "monospace",
        }}>
          Health: {healthPct}
        </span>
      </div>
      ...
      {changes.length === 0 ? (
        <div style={{
          ...
          <div style={{ fontSize: 13, marginBottom: 8 }}>No instruction changes detected</div>
          <div style={{ fontSize: 11 }}>
            This view shows real-time diffs of CLAUDE.md and rules file changes.
            Edit an instruction file while Onlooker is running to see changes appear here.
          </div>
          {health?.cwd && (
            <div style={{ fontSize: 10, color: C.textMuted, marginTop: 12, fontFamily: "monospace" }}>
              Watching: {health.cwd}
            </div>
          )}
        </div>
        ...
        <>
          <div style={{
            fontSize: 10, color: C.textMuted, marginBottom: 14,
          ...
          {changes.map((ch, i) => (
            <DiffView key={`${ch.file}-${ch.ts}-${i}`} change={ch} />
          ))}
        </>
      )}
    </div>
