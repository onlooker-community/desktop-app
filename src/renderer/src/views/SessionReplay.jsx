...
import { PLUGIN_COLORS, PLUGIN_LABELS, STATUS_COLORS, pluginColor } from "../plugins.js";
        ...
        {sessions.slice(0, 50).map((s) => {
          ...
          return (
            <div key={s.id} onClick={() => onSelect(s.id)} style={{
              ...
              onMouseLeave={(e) => { if (!isSelected) e.currentTarget.style.background = isSelected ? `${C.pink}0f` : "transparent"; }}
            >
              <div style={{
                fontSize: 9, color: C.textMuted, fontFamily: "monospace", marginBottom: 3,
              }}>
                {s.id?.slice(0, 14)}…
              </div>
              <div style={{ fontSize: 10, color: C.textSecondary }}>
                {s.start ? new Date(s.start).toLocaleDateString("en-US", {
...
function PlaybackControls({ position, total, playing, speed, onSeek, onToggle, onSpeed, onStep }) {
  return (
    <div style={{
      ...
      {/* Step back */}
      <button onClick={() => onStep(-1)} title="Previous event" style={{
      ...
      {/* Play/pause */}
      <button onClick={onToggle} title={playing ? "Pause" : "Play"} style={{
        fontSize: 14, border: `1px solid ${C.border}`, borderRadius: 6,
        background: playing ? `${C.pink}22` : "transparent",
        color: playing ? C.pink : C.textSecondary,
        cursor: "pointer", padding: "4px 10px", width: 36,
      }}>
        {playing ? "⏸" : "▶"}
      </button>

      {/* Step forward */}
      <button onClick={() => onStep(1)} title="Next event" style={{
        fontSize: 12, border: "none", background: "transparent",
        color: C.textSecondary, cursor: "pointer", padding: "2px 6px",
      }}>
        ⏭
...
function TimelineBar({ events, position }) {
  ...
  return (
    <div style={{
      padding: "0 16px", background: C.bg1,
      borderBottom: `1px solid ${C.border}`,
    }}>
      <svg width={w} height={h} style={{ display: "block" }} viewBox={`0 0 1000 ${h}`}>
        {events.map((e, i) => {
        ...
        <rect
          ...
          y={0} width={2} height={h} fill={C.pink}
        />
      </svg>
    </div>
  );
    ...
    <div style={{ padding: "16px 20px" }}>
      ...
      <div style={{
        ...
        <span style={{
          ...
          border: `1px solid ${plugColor}44`, color: plugColor,
      ...
      {event.detail && (
        <div style={{
          ...
          padding: "8px 10px", background: C.bg0, borderRadius: 5,
      ...
      {/* Stats bar */}
      <div style={{
        display: "flex", gap: 16, fontSize: 10, color: C.textMuted,
        padding: "8px 10px", background: C.bg2, borderRadius: 6,
        border: `1px solid ${C.border}`, marginBottom: 16,
      }}>
        <span>Status: <span style={{ color: statusColor }}>{event.status}</span></span>
        {event.tool_name && <span>Tool: <span style={{ color: C.cyan }}>{event.tool_name}</span></span>}
        {cumCost > 0 && <span>Cum. Cost: <span style={{ color: C.green }}>{fmtCost(cumCost)}</span></span>}
      </div>
...
export default function SessionReplay({ sessions }) {
  ...
  const { cumCost, filesThisTurn } = useMemo(() => {
    ...
    for (let i = 0; i <= position && i < events.length; i++) {
      ...
      if (filePath) {
        const type = (toolName === "Write" || toolName === "Edit") ? "write" : "read";
  ...
  const step = useCallback((dir) => {
    setPosition((prev) => Math.max(0, Math.min(events.length - 1, prev + dir)));
  }, [events.length]);
