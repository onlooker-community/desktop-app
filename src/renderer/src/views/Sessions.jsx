				...
				{session.friction != null && (
					<span
						style={{
							...
							fontWeight: 600,
							color:
								session.friction.score < 0.3
									? C.green
									: session.friction.score < 0.6
										? C.yellow
										: C.red,
						}}
					>
	...
	return (
		...
		>
			{plugins.map((plugin) => {
				...
				const warns = pluginEvents.filter(
					(e) => e.status === "warn" || e.status === "fail",
				).length;
				const passes = pluginEvents.filter(
				...
				return (
					...
					>
						...
						<div style={{ fontSize: 9, color: C.textMuted }}>
							{blocks > 0 && (
								<span style={{ color: C.red }}>⊘ {blocks} block </span>
							)}
							{warns > 0 && (
								<span style={{ color: C.yellow }}>⚑ {warns} warn </span>
							)}
...
function ScoreSparkline({ events }) {
	...
	const xs = scored.map(
		(_, i) => pad + (i / (scored.length - 1)) * (w - pad * 2),
	);
	const ys = scored.map(
		(s) => h - pad - ((s - min) / (max - min)) * (h - pad * 2),
	);
	const path = xs
	...
	return (
		<div style={{ marginBottom: 20 }}>
			...
			<svg width={w} height={h} style={{ display: "block" }}>
				...
				{xs.map((x, i) => (
					<circle
						key={`score-${x.toFixed(1)}-${scored[i]}`}
						cx={x}
						cy={ys[i]}
						r={2.5}
...
function DetailEventRow({ event }) {
	...
	return <div style={{ padding: "5px 0" }}>{inner}</div>;
}
...
function SessionDetail({ session }) {
	...
	const hasTurnStructure =
		turns.length > 0 && (turns.length > 1 || turns[0]?.toolCalls?.length > 0);
				...
				<div style={{ marginBottom: 20 }}>
					...
					<div style={{ display: "flex", alignItems: "center", gap: 12 }}>
						...
						<div style={{ fontSize: 11, color: C.textMuted }}>
							...
							{session.blocks > 0 && (
								<span style={{ marginLeft: 10, color: C.red }}>
							...
							{[
								...
								<button
									...
									style={{
										...
										border: "none",
										borderRight:
											mode === "turns" ? `1px solid ${C.border}` : "none",
										background:
											viewMode === mode ? `${C.pink}20` : "transparent",
