...
import { PLUGIN_COLORS, pluginColor } from "../plugins.js";
import { useCostData, computeFriction } from "../hooks/useOnlooker.js";
...
function detectAnomalies(sessions, costRecords) {
  if (sessions.length < 10) return { anomalies: [], baseline: null, reason: "insufficient" };
  ...
  // Compute baselines
  const costs = sessions.map((s) => costBySession[s.id] ?? 0).filter((c) => c > 0);
  const scores = sessions.filter((s) => s.avgScore != null).map((s) => s.avgScore);
  const wardenCounts = sessions.map((s) =>
    (s.events ?? []).filter((e) => e.plugin === "warden" && e.status === "block").length
  );
  ...
  const baseline = { costStats, scoreStats, wardenStats, durationStats, frictionStats };
  const anomalies = [];
  ...
  for (const session of sessions) {
    ...
    if (scoreStats.std > 0 && session.avgScore != null &&
    ...
    if (wardenStats.std > 0 && wardenCount > wardenStats.mean + 3 * wardenStats.std) {
      signals.push({
        signal: "warden",
    ...
    // Friction > 2σ
    if (session.friction != null && frictionStats.std > 0 &&
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <span style={{
          fontSize: 10, color: C.textSecondary, fontFamily: "monospace",
        }}>
          {session.id?.slice(0, 16)}…
        </span>
        <span style={{ fontSize: 10, color: C.textMuted }}>
          {session.start ? new Date(session.start).toLocaleDateString("en-US", {
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {signals.map((s) => (
            <span key={s.signal} title={s.label} style={{
              fontSize: 8, fontFamily: "monospace", padding: "1px 4px",
              borderRadius: 3, background: `${sevColor(s.severity)}15`,
      ...
      {expanded && (
        <div style={{
          marginTop: 8, padding: "8px 10px", background: C.bg0,
          borderRadius: 5, border: `1px solid ${C.border}`,
        }}>
          {signals.map((s, i) => (
            <div key={i} style={{
              fontSize: 10, color: C.textSecondary, marginBottom: 4,
              display: "flex", alignItems: "center", gap: 6,
            }}>
              <span style={{
                width: 6, height: 6, borderRadius: "50%",
                background: sevColor(s.severity), flexShrink: 0,
              }} />
              {s.label}
            </div>
          ))}
          <div style={{
            fontSize: 9, color: C.textMuted, marginTop: 6, fontFamily: "monospace",
          }}>
    ...
    <div style={{ height: "100%", overflowY: "auto", padding: "20px 24px" }}>
      ...
      {reason === "insufficient" ? (
        <div style={{
          ...
          <div style={{ fontSize: 13, marginBottom: 8 }}>Not enough data yet</div>
          <div style={{ fontSize: 11 }}>
            Anomaly detection needs at least 10 sessions to establish a baseline.
            Currently have {sessions.length} session{sessions.length !== 1 ? "s" : ""}.
          </div>
        </div>
      ) : (
        <>
          ...
          {baseline && (
            <div style={{
              ...
              {[
                ...
                { label: "Avg Score", value: baseline.scoreStats.mean > 0 ? baseline.scoreStats.mean.toFixed(2) : "—" },
                { label: "Avg Friction", value: baseline.frictionStats.mean > 0 ? baseline.frictionStats.mean.toFixed(2) : "—" },
                { label: "Avg Duration", value: baseline.durationStats.mean > 0 ? `${Math.round(baseline.durationStats.mean)}m` : "—" },
              ].map((b) => (
                <div key={b.label} style={{ fontSize: 10 }}>
                  <span style={{ color: C.textMuted }}>{b.label}: </span>
                  <span style={{ color: C.textSecondary, fontFamily: "monospace" }}>{b.value}</span>
                </div>
              ))}
            </div>
          )}
          ...
          {anomalies.length === 0 ? (
            <div style={{
              textAlign: "center", padding: "40px 0", color: C.green, fontSize: 12,
            }}>
