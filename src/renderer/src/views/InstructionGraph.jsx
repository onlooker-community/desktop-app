...
import { useInstructionGraph, useInstructionWatcher } from "../hooks/useOnlooker.js";

const C = {
  bg0: "#0b0d14", bg1: "#12151f", bg2: "#181c2a", bg3: "#1f2335",
  border: "#252a3d",
  pink: "#f472b6", cyan: "#22d3ee", yellow: "#fbbf24",
  green: "#4ade80", red: "#f87171", sky: "#38bdf8",
...
function useForceLayout(nodes, edges, width, height) {
  ...
  useEffect(() => {
    if (!nodes || nodes.length === 0) { setPositions([]); return; }

    // Initialize positions in a circle
    const pos = nodes.map((_, i) => ({
      x: width / 2 + (width * 0.35) * Math.cos((2 * Math.PI * i) / nodes.length),
      y: height / 2 + (height * 0.35) * Math.sin((2 * Math.PI * i) / nodes.length),
      vx: 0,
      vy: 0,
    }));
      ...
      <div ref={containerRef} style={{ flex: 1, minHeight: 400, position: "relative" }}>
        <svg width={dimensions.width} height={dimensions.height} style={{ display: "block" }}>
          ...
          {edges.map((edge, i) => {
            ...
            const color = edge.type === "contradiction" ? C.red : `${C.textMuted}44`;
            return (
              <line key={i}
          ...
          {nodes.map((node, i) => {
            ...
            const color = isFile
              ? (hasIssues ? C.yellow : C.sky)
              : sevColor(node.severity);
            const r = isFile ? 18 : 10;

            return (
              <g key={node.id} onClick={() => setSelected(node.id)}
                ...
                {isFile && (
                  <text
                    x={positions[i].x} y={positions[i].y + r + 12}
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span
              ...
              style={{ fontSize: 10, color: C.textMuted, cursor: "pointer" }}
            >
              ✕
            </span>
          </div>

          <div style={{ fontSize: 9, color: C.textMuted, fontFamily: "monospace", marginBottom: 8 }}>
          ...
          {selectedNode.issues > 0 && (
            <div style={{ fontSize: 10, color: C.yellow }}>
              {selectedNode.issues} issue{selectedNode.issues !== 1 ? "s" : ""} found
            </div>
          )}
        </div>
      )}
  ...
  return (
    <div style={{
      ...
      {changes.slice(0, 10).map((ch, i) => (
        <div key={i} style={{
          padding: "6px 0", borderBottom: i < changes.length - 1 ? `1px solid ${C.border}` : "none",
...
export default function InstructionGraph() {
  ...
  const healthPct = graph?.health_score != null
    ? `${Math.round(graph.health_score * 100)}%`
    : "—";
  const healthColor = graph?.health_score != null
    ? (graph.health_score >= 0.85 ? C.green : graph.health_score >= 0.7 ? C.yellow : C.red)
    : C.textMuted;

  const totalIssues = graph?.issue_count
    ? (graph.issue_count.high ?? 0) + (graph.issue_count.medium ?? 0) + (graph.issue_count.low ?? 0)
    : 0;
    ...
    <div style={{ height: "100%", overflowY: "auto", padding: "20px 24px" }}>
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{
            ...
            border: `1px solid ${C.sky}44`, color: C.sky,
            background: `${C.sky}11`, fontFamily: "'JetBrains Mono', monospace",
          }}>
            CAR
          </span>
        </div>
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <div style={{ fontSize: 13, marginBottom: 8 }}>No instruction files found</div>
          <div style={{ fontSize: 11 }}>
            The instruction graph appears once Cartographer has audited your CLAUDE.md and rules files.
            Run a Claude Code session with Cartographer installed to generate the data.
          </div>
        </div>
      ) : (
        <>
          {/* Legend */}
          <div style={{
            display: "flex", gap: 16, marginBottom: 14, fontSize: 9, color: C.textMuted,
            padding: "6px 10px", background: C.bg2, borderRadius: 6,
            border: `1px solid ${C.border}`,
          }}>
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <svg width="12" height="12"><circle cx="6" cy="6" r="5" fill={`${C.sky}33`} stroke={C.sky} /></svg>
              File
            </span>
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <svg width="12" height="12"><circle cx="6" cy="6" r="4" fill={`${C.red}33`} stroke={C.red} /></svg>
              Issue
            </span>
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <svg width="20" height="12"><line x1="0" y1="6" x2="20" y2="6" stroke={C.red} strokeWidth="2" /></svg>
              Contradiction
            </span>
            <span style={{ display: "flex", alignItems: "center", gap: 4 }}>
              <svg width="20" height="12"><line x1="0" y1="6" x2="20" y2="6" stroke={C.textMuted} strokeDasharray="4,3" /></svg>
              Reference
            </span>
