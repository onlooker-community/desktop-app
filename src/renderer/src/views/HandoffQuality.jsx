...
const C = {
  ...
  green: "#4ade80", red: "#f87171", emerald: "#34d399",
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <span style={{
          ...
          color, width: 36, textAlign: "center",
        }}>
          {handoff.score}%
        </span>
        ...
        <span style={{ fontSize: 10, color: C.textMuted }}>
          {handoff.handoff_ts ? new Date(handoff.handoff_ts).toLocaleDateString("en-US", {
        ...
        <div style={{ display: "flex", gap: 8, fontSize: 9, fontFamily: "monospace" }}>
          {handoff.resolved > 0 && (
            <span style={{ color: C.green }}>✓ {handoff.resolved} resolved</span>
          )}
          {handoff.persisted > 0 && (
            <span style={{ color: C.yellow }}>↻ {handoff.persisted} persisted</span>
          )}
      ...
      {expanded && (
        <div style={{
          ...
          <div style={{ display: "flex", gap: 16, fontSize: 10, color: C.textSecondary }}>
            <span>Total questions: {handoff.total_questions}</span>
            <span>Resolved: <span style={{ color: C.green }}>{handoff.resolved}</span></span>
            <span>Persisted: <span style={{ color: C.yellow }}>{handoff.persisted}</span></span>
          </div>
          {handoff.next_session_id && (
            <div style={{ fontSize: 9, color: C.textMuted, marginTop: 4, fontFamily: "monospace" }}>
  ...
  return (
    <div style={{
      ...
      <svg width="100%" viewBox={`0 0 ${w} ${h}`} style={{ display: "block" }}>
        ...
        <path d={line} fill="none" stroke={C.emerald} strokeWidth={2}
          strokeLinecap="round" strokeLinejoin="round" />
        {xs.map((x, i) => (
          <circle key={i} cx={x} cy={ys[i]} r={3} fill={qualityColor(points[i].score)} />
        ))}
      </svg>
    </div>
  );
...
export default function HandoffQuality() {
  ...
  const avgScore = useMemo(() => {
    const scored = handoffs.filter((h) => h.score != null && h.total_questions > 0);
    return scored.length > 0
      ? Math.round(scored.reduce((s, h) => s + h.score, 0) / scored.length)
      : null;
  }, [handoffs]);

  const worstCount = handoffs.filter((h) => h.score < 50 && h.total_questions > 0).length;
  const totalResolved = handoffs.reduce((s, h) => s + h.resolved, 0);
  const totalPersisted = handoffs.reduce((s, h) => s + h.persisted, 0);
    ...
    <div style={{ height: "100%", overflowY: "auto", padding: "20px 24px" }}>
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{
            ...
            border: `1px solid ${C.emerald}44`, color: C.emerald,
            background: `${C.emerald}11`, fontFamily: "'JetBrains Mono', monospace",
          }}>
            RLY
          </span>
        </div>
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <div style={{ fontSize: 13, marginBottom: 8 }}>No handoffs recorded</div>
          <div style={{ fontSize: 11 }}>
            Handoff quality data appears once Relay captures session handoffs.
            Quality is measured by comparing handoff documents to the next session's activity.
          </div>
        </div>
      ) : (
        <>
          <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
            <StatCard label="Avg Quality" value={avgScore != null ? `${avgScore}%` : "—"}
              color={qualityColor(avgScore)} />
            <StatCard label="Total Handoffs" value={handoffs.length} />
            <StatCard label="Questions Resolved" value={totalResolved}
              color={C.green} />
            <StatCard label="Low Quality (<50%)" value={worstCount}
          ...
          <div style={{
            ...
            {handoffs.map((h, i) => (
              <HandoffRow key={`${h.session_id}-${i}`} handoff={h} />
            ))}
          </div>
        </>
      )}
